...
**File:** `src/lib/itinerariesData.ts`

```typescript
// Add to the itineraries array:
{
  slug: "new-itinerary",                    // URL-friendly name
  title: "Canyon Adventure",                // Display name
  category: "Adventure",                    // Type of tour
  description: "Explore the beautiful...",  // Short description
  heroImage: require("@/assets/..."),      // Banner image
  fullDescription: "Long description...",   // Full details
  duration: "5 Days",                       // Trip length
  price: "$1,999",                          // Cost
  includes: ["Meals", "Guide"],             // What's included
}
```
...
**Step 1:** Create a folder in `src/app/`
...
**Step 3:** Access it at `https://yoursite.com/new-page`
...
We use **Tailwind CSS** for styling. It's a utility-first CSS framework.

**How it works:**
Instead of writing custom CSS, we add class names to HTML elements.
...
| What        | Class                                   | Example                    |
| ----------- | --------------------------------------- | -------------------------- |
| **Padding** | `p-4`, `px-2`, `py-3`                   | Adds space inside element  |
| **Margins** | `m-4`, `mx-2`, `my-3`                   | Adds space outside element |
| **Display** | `flex`, `grid`, `block`                 | How element is arranged    |
| **Colors**  | `bg-blue-600`, `text-white`             | Background and text colors |
| **Sizing**  | `w-full`, `h-dvh`, `size-12`            | Width and height           |
| **Rounded** | `rounded`, `rounded-lg`, `rounded-full` | Corner radius              |
| **Shadows** | `shadow-md`, `shadow-xl`                | Drop shadows               |
| **Hover**   | `hover:bg-blue-700`                     | Effect on mouse hover      |

### Dark Mode

We use the `dark:` prefix for dark mode styles:
...
For styles Tailwind doesn't have, we use CSS files:
...
1. **User fills form** on `/contacto` page
2. **Form validates:**
   - Name (letters and spaces only)
   - Email (valid email format)
   - Phone (10-13 digits)
   - reCAPTCHA (spam protection)
3. **User clicks "Enviar" (Send)**
4. **Form submits to API endpoint** (`/api/send-email`)
5. **Server sends email** with form data
6. **If successful:** Redirect to `/gracias` (thank you page)
7. **If error:** Show error message on `/error-contacto`

### Form Component

**File:** `src/sections/nosotros/Contacto.tsx`

**Key features:**

- Input validation while typing
- Real-time cleaning of text (removes special characters)
