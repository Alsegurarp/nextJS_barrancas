...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  return (
    <div
      className={`sm-scope fixed top-0 left-0 w-full h-full overflow-hidden z-40`}
    ...
    >
      ...
      >
          ...
          >
            ...
            >
              <span ref={textInnerRef} className="sm-toggle-textInner flex flex-col leading-none">
                {textLines.map((l, i) => (
                  <span className="sm-toggle-line block h-[2.2em] leading-none" key={i}>
                    {l}
                  </span>
                ))}
              </span>
        ...
        <aside
          ...
          ref={panelRef}
          className="staggered-menu-panel absolute top-0 right-0 h-full bg-white flex flex-col p-[6em_2em_2em_2em] overflow-y-auto z-50 backdrop-blur-[12px] shadow-[-2px_0px_20px_2px_rgba(255,_102,_0,_0.15)]"
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={`sm-item-${idx}`}>
                    {/* Render different label types:
                        - string/number: render as an anchor with href (keeps previous behavior)
                        - valid React element: clone it to inject accessibility props
                        - other nodes: render inside a div
                    */}
                    ...
                    ) : React.isValidElement(it.label) ? (
                      // If label is a React element, clone it to add aria or data-index if possible
                      React.cloneElement(
                        it.label as React.ReactElement,
                        ({
                          'aria-label': it.ariaLabel,
                          'data-index': idx + 1,
                          className: `${(((it.label as React.ReactElement).props as any)?.className) || ''} sm-panel-item`,
                        } as any)
                      )
                    ) : (
                      <div
                        className="sm-panel-item relative text-black font-semibold text-[4rem] leading-none tracking-normal uppercase inline-block pr-[1.4em]"
                        aria-label={it.ariaLabel}
                        data-index={idx + 1}
                      >
                        <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform">
            ...
            {displaySocials && socialItems && socialItems.length > 0 && (
              <div className="sm-socials mt-auto mb-[10vh] pt-8 flex flex-col gap-3" aria-label="Social links">
                ...
                >
                  {socialItems.map((s, i) => (
                    <li key={`sm-social-${i}`} className="sm-socials-item">
                      <a
                        href={s.link}
                        target="_blank"
      ...
      {/* .sm-scope .staggered-menu-header THIS CLASS CONTROLS THE BACKGROUND COLOR OF THE MENU*/}
      <style>{`
.sm-scope .staggered-menu-wrapper { position: relative; width: 100%; height: 100%; z-index: 40; }
.sm-scope .staggered-menu-header { position: absolute; top: 0; left: 0; width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 0.8em; background: white; pointer-events: none; z-index: 20; }
...
.sm-scope .sm-socials-link { font-size: 1.8rem; font-weight: 500; color: #111; text-decoration: none; position: relative; padding: 2px 0; display: inline-block; transition: color 0.3s ease, opacity 0.3s ease; }
.sm-scope .sm-socials-link:hover { color: var(--sm-accent, #ff0000); }
.sm-scope .sm-panel-title { margin: 0; font-size: 1.2rem; font-weight: 600; color: #fff; text-transform: uppercase; }
