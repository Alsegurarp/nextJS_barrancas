...
function LogoCarousel({ direction = 'rtl' }: LogoCarouselProps) {
  ...
  return (
    <div className="container-full overflow-hidden cursor-pointer backdrop-blur-sm py-2 sm:py-10 md:py-16 flex justify-center bg-white">
        <motion.div 
            className="flex"
            style={{
                width: 'fit-content',
            ...
            {extendedLogos.map((item, index) => (
                <div className="h-12 w-12 sm:h-14 sm:w-14 md:h-16 md:w-16 lg:h-20 lg:w-20 flex items-center justify-center object-contain shrink-0" key={index}>
                    {item}
                </div>
            ))}
        </motion.div>
